...
class Puzzle19(private val enforceDecisions: EnforcedDecisions = EnforcedDecisions()) : Puzzle<Int> {
    ...
    class Blueprint(
        ...
        fun getScore(): Int {
            return bestSimulation!!.getGeodes() * id
        }
    ...
    class EnforcedDecisions {
        ...
        fun inRound(round: Int, decision: ResourceType): EnforcedDecisions {
            roundToDecision[round] = decision
            return this
        }
        ...
        fun allOtherRoundsNone(): EnforcedDecisions {
            for (round in 1..ROUNDS) {
                if (!roundToDecision.containsKey(round)) {
                    roundToDecision[round] = null
                }
            }
            return this
        }
    ...
    class Simulation(
        ...
        fun run(): Simulation? {
            ...
            if (blueprint.bestSimulation!!.geodesInRound[round] > resources[GEODE]!! + robots[GEODE]!!) {
                //println("EL2")
                return null
            } // Won't beat best simulation
    ...
    override fun solve(lines: List<String>): Int {
        blueprints = lines.map { Blueprint.fromString(it, enforceDecisions) }
        var result = 0
        for (blueprint in blueprints) {
            println("Evaluating blueprint ${blueprint.id}")
            blueprint.evaluate()
            result += blueprint.getScore()
        }
        return result
    }
}
...
fun main() {
    val result = Puzzle19().solveForFile()
    println("---")
    println(result)
}